	...
	useEffect(() => {
		...
		fetch(url, {
			...
			.then((data) => {
				...
				const formattedData: Data[] = data.Bin.map((item: any) => { // Note data.Bin aqui
					let coordenadasFormatadas = "N/A";

					try {
						if (item.coordenadas) {
							const coordenadasObj = JSON.parse(item.coordenadas);
					...
					return {
						...
						nomeMedicamento: item.nomeMedicamento || "N/A",
						quantidade: item.quantidade || 0,
						coordenadas: coordenadasFormatadas, // Corrigido a digitação aqui
					};
				});
			...
			<div className='flex justify-between items-center'>
				...
				<Stack id="botoes" spacing={1} direction="row">
					<Button
						...
						onClick={async () => {
							try {
								const response = await fetch(`${apiUrl}/robot/getRobotCoordinates`, {
									headers: {
										"ngrok-skip-browser-warning": "true",
										"User-Agent": "Custom-User-Agent",
										"Authorization": `Bearer ${document.cookie.split('token=')[1]}`
